...
@property (nonatomic, strong)NSMutableArray * rankArray; // 用来存放所有的排行榜对象
...
- (void)viewDidLoad {
    ...
    // 注册cell
    UINib * nib = [UINib nibWithNibName:@"RankListTableViewCell" bundle:nil];
    [self.tableView registerNib:nib forCellReuseIdentifier:@"RankListTableViewCell"];
    
    // 调用网络请求的方法
//    [self sendRankListDataRequest];
...
- (void)sendRankListDataRequest
{
    // 网址请求
    NSURL * url = [NSURL URLWithString:kRankList((long)self.count)];
    // 请求对象
    NSURLRequest * request = [NSURLRequest requestWithURL:url];
    // 链接对象
    __weak RankListTableViewController * weakTVC = self;
...
//            NSLog(@"%@", array);
            for (NSDictionary * dic in array) {
                RankListModel * rankList = [[RankListModel alloc] init];
                [rankList setValuesForKeysWithDictionary:dic];
                [weakTVC.rankArray addObject:rankList];
            }
        }
        // 刷新数据
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    model = self.rankArray[indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    RankListModel *model = self.rankArray[indexPath.row];
    [self url:model.vid];
...
-(void)url:(NSString *)vid
{
    ...
    NSURLRequest * request = [NSURLRequest requestWithURL:url];
    // 链接对象
    __weak RankListTableViewController * latestTVC = self;
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        if (data != nil) {
